...
describe('Docker build, login & push commands', () => {
  ...
  describe('Registry login', () => {
    ...
    test('Docker Hub login', () => {
      ...
      expect(cp.execFileSync).toHaveBeenCalledWith('docker', ['login', '-u', username, '--password-stdin', registry], {
        input: password
      });
    });
    ...
    test('ECR login', () => {
      ...
      expect(cp.execFileSync).toHaveBeenCalledWith('aws', ['ecr', 'get-login-password', '--region', 'us-east-1']);
