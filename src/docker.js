...
const login = (username, password, registry) => {
  ...
  if (isEcr(registry)) {
    ...
    const ecrPassword = cp.execFileSync('aws', ['ecr', 'get-login-password', '--region', region]);
    cp.execFileSync('docker', ['login', '--username', 'AWS', '--password-stdin', registry], { input: ecrPassword });
  } else if (username && password) {
    core.info(`Logging into Docker registry ${registry}...`);
    cp.execFileSync('docker', ['login', '-u', username, '--password-stdin', registry], { input: password });
  } else {
